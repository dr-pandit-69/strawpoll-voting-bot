...
def get_csrf_token_and_vote_count(session, url, proxies, csrf_token_regex, vote_count_file):
...
def send_vote(url, csrf_token, session, vote_data, proxies, max_retries=3):
    headers = {
    ...
    for retry in range(max_retries):
        try:
            response = session.post(url, json=vote_data, headers=headers, proxies=proxies)
            if response.status_code == 200:
...
def update_vote_count(count, vote_count_file,vote_option_name):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(vote_count_file, 'a') as file:
        file.write(f"{count} Given to {vote_option_name} at {timestamp}\n")
...
# Renew Tor circuit to change IP
def renew_tor_circuit(tor_password):
...
def main():
    ...
    proxies = {
        'http': f'socks5h://{vote_details["proxy"]}',
        'https': f'socks5h://{vote_details["proxy"]}'
    }
    ...
    with requests.Session() as session:
        ...
        csrf_token, vote_count = get_csrf_token_and_vote_count(
            session, vote_details["url"], proxies, 
        ...
        for _ in range(vote_details["max_votes"]):  # Number of votes to cast
            if send_vote(
                ...
                vote_details["vote_data"], proxies, vote_details["retry_attempts"]
            ):
                vote_count += 1
                update_vote_count(vote_count, vote_details["vote_count_file"],vote_details["vote_option_name"])
            renew_tor_circuit(vote_details["tor_password"])
